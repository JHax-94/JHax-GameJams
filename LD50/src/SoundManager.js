...
export default class SoundManager
{
    ...
    Update(deltaTime)
    {
        if(this.soundOn)
        {
            //consoleLog("Updating sound manager...");

            this.songElapsed += deltaTime;

            if(this.rampModeOn)
            {
                if(this.tensionLevel + 1 < this.levelMusic.length)
                {
                    if(this.songElapsed > this.trackTime - this.fadeoutTime && this.alarmDelayTime <= 0 && this.trackDelayTime <= 0)
                    {
                        let timeIntoFade = this.songElapsed - (this.trackTime - this.fadeoutTime);

                        let fadePortion = 1 - (timeIntoFade / this.fadeoutTime);
                        //consoleLog(`Fade portion: ${fadePortion}`);

                        if(fadePortion < 0)
                        {
                            fadePortion = 0;
                            this.alarmDelayTime = this.alarmDelay;
                        }

                        this.SetMusicVolume(fadePortion * this.musicVolume);
                    }
                }
                ...
                if(this.alarmDelayTime > 0)
                {
                    ...
                    if(this.alarmDelayTime <= 0)
                    {
                        ...
                        this.trackDelayTime = this.trackDelay;
                    }
                }
